...
function LoginPage() {
  ...
  const submitEmail = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Hit the new Appwrite serverless function for passwordless token generation
      // Make sure VITE_LOGIN_FUNCTION_ID is set in your .env or replace it with your actual Function ID
      ...
      if (response.status === 'failed') {
        const errorMsg = response.responseBody || "Failed to execute login function";
        console.error("Function Error:", errorMsg);
        throw new Error("Failed to login. Ensure you are an authorized user.");
      }

      // Parse the successful response from the Appwrite function
      let data;
      try {
