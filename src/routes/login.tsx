...
function LoginPage() {
  ...
  const submitEmail = async (e: React.FormEvent) => {
    ...
    try {
      // Hit the new custom Express backend endpoint for passwordless token generation
      // Replace localhost:5000 with the actual backend URL when deployed
      const backendUrl = import.meta.env.VITE_BACKEND_URL || "http://localhost:5000";
      const response = await fetch(`${backendUrl}/api/auth/appwrite-login`, {
        method: "POST",
      ...
      if (!response.ok) {
        throw new Error(data.error || "Failed to login. Ensure you are an authorized user.");
      }

      // Appwrite token successfully generated by our secure backend!
      // Now use the Appwrite Client to establish the session.
      await account.createSession(data.userId, data.secret);
      
      // Reload without query params to trigger auth state refresh
      window.location.replace(window.location.pathname);
    } catch (err: any) {
      console.error(err);
      setError(err.message || "Failed to login via Email.");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <AppShell>
      ...
      <div className="relative z-10 mx-auto max-w-md w-full px-4 py-1 my-auto flex flex-col justify-center">
        <div className="mb-2 overflow-hidden rounded-2xl bg-white/60 dark:bg-slate-900/60 backdrop-blur-xl border border-white/40 dark:border-white/10 p-2.5 shadow-lg">
            ...
            <div className="space-y-3">
              <div className="relative">
                <input
                  ...
                  className="w-full rounded-xl border border-border bg-background pl-4 pr-4 py-3 text-[0.95rem] font-medium outline-none transition-all focus:border-primary focus:ring-2 focus:ring-primary/20"
                  required
                  disabled={loading}
                />
              </div>
